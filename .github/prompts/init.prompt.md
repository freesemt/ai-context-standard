...
`ai-context-standard` リポジトリの `.github/vscode-version.txt`（このプロンプトファイルと同じリポジトリ）を読んでください。
...
取得したバージョンの数値部分（例: `1.115.0` の `1.115`）が **1.99 以上** かどうか判定してください。
...
最後に「**初期化完了** (ai-context-standard) / VS Code vX.XX.X ✅」と明記してください。
